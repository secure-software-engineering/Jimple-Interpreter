...
3. Run `mvn install` int the *root* directory<sup>1</sup> 
...
7. You are ready to go! Try to run the test cases in `src/systemTest/java` inside your IDE. 11 of the test should already be passing!
...
###### Hints 
- There are currently no test for exceptional control flow. Feel free to add some!
- The `interpret` methods in `de.upb.soot.jimple.interpreter.JimpleInterpreter` will probably need a rework to follow the concrete control flow
...
- Implement generics 
- Fix class and short constants
    - short constants do not work since we do not interprete the right side of an assignment dependent on the type of the left side in `de.upb.soot.jimple.interpreter.StmtInterpreter.caseAssignStmt`
...
- Implement possibility to start interpretation in the middle of an (instance) method.
...
- [objenesis](https://github.com/easymock/objenesis) is a library that allows to construct Java objects without calling the constructor
    - we need exactly this to not execute the constructor since this coud a) diverge object state and b) introduce duplicate execution
...
3. ...if not, `alt+enter` with the cursor on the class name and select `reate Test`. You will be presented with a dialog like this one:
